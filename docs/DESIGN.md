...
> Design notes for **media-webhook-retry-queue** — a runnable rust example that Queue media events and deliver their webhooks with Rust acknowledgement-based retries.
...
- **Thin client** — a ~30-line helper that owns the base URL, the auth header, and envelope unwrapping, so call sites stay readable (e.g. `infrai.queue.publish(...)`).
- **Feature code** — the actual task: media-webhook-retries.
